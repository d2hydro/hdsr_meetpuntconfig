...
# controle op KW/OW
logging.info("check if KW/OW locations are in correct section")
consistency_df["idmap v sectie"] = pd.DataFrame(
...
for section_type, sections in idmap_subsecs.items():
    for section in sections:
        ...
        idmap_wrong_section = [
            idmap
            for idmap in xml_to_dict(fews_config.IdMapFiles[idmap], **section)["idMap"]["map"]
        ...
        if len(idmap_wrong_section):
            ...
            logging.warning(
                f"{len(idmap_wrong_section)} internalLocations anders dan {prefix}XXXXXX tussen {section_start} en {section_start} in {idmap}"
            )
...
hist_tags_org_df = pd.read_csv(hist_tags_csv, parse_dates=dtype_cols, sep=";")

for col in dtype_cols:
    if not pd.api.types.is_datetime64_dtype(hist_tags_org_df[col]):
        logging.error(
            f"kolom '{col}' in '{hist_tags_csv}' kan niet worden geconverteerd"
...
hist_tags_no_match_df = hist_tags_no_match_df[
    ~hist_tags_no_match_df["serie"].isin(consistency_df["histTag_ignore"]["UNKNOWN_SERIE"])
]
...
if not consistency_df["histTags_noMatch"].empty:
    logging.warning(f"{len(consistency_df['histTags_noMatch'])} histTags zijn niet opgenomen in idmap")
else:
...
hist_tag_ignore_match_df = consistency_df["histTag_ignore"][
    consistency_df["histTag_ignore"]["UNKNOWN_SERIE"].isin(hist_tags_opvlwater_df["serie"])
]
...
if not consistency_df["histTags_ignore_match"].empty:
    logging.warning(
        f"{len(consistency_df['histTags_ignore_match'])} histTags zijn ten onrechte opgenomen in histTag ignore"
    )
else:
...
h_locs_missing = [loc for loc in h_locs if iloc not in list(mpt_df.index)]
...
consistency_df["dubbele idmaps"] = pd.DataFrame(
    columns=["bestand", "externalLocation", "externalParameter", "internalLocation", "internalParameter",]
)
for idmap_file in idmap_files:
    ...
    if len(idmap_doubles) > 0:
        ...
        df = pd.DataFrame(
            idmap_doubles, columns=["internalLocation", "externalLocation", "internalParameter", "externalParameter",],
        )
        df["bestand"] = idmap_file
        consistency_df["dubbele idmaps"] = pd.concat([consistency_df["dubbele idmaps"], df], axis=0)
        logging.warning(f"{len(idmap_doubles)} dubbele idmap(s) in {idmap_file}")
    else:
        logging.info(f"geen dubbele idmaps in {idmap_file}")
...
if len(params_missing) == 0:
    ...
    logging.warning(f"{len(params_missing)} parameter(s) in idMaps missen in config")
...
for loc_id, gdf in grouper:
    ...
    if any([re.match(loc, loc_id) for loc in xy_ignore_df["internalLocation"]]):
        fields["X"], fields["Y"] = next(
            [row["x"], row["y"]] for index, row in xy_ignore_df.iterrows() if re.match(row["internalLocation"], loc_id)
        )
    else:
...
if consistency_df["hloc error"].empty:
    ...
    hoofdloc_gdf["geometry"] = hoofdloc_gdf.apply((lambda x: Point(float(x["X"]), float(x["Y"]))), axis=1)
    hoofdloc_gdf = hoofdloc_gdf[columns]


else:
    logging.warning(f"{len(consistency_df['hloc error'])} fouten bij aanmaken hoofdlocaties")
...
if "externalParametersAllowed" in config.keys():
    expars_allowed = {
        key: value.replace(" ", "").split(",") for key, value in config["externalParametersAllowed"].items()
    }
...
for int_loc, loc_group in idmap_df.groupby("internalLocation"):
    ...
    if loc_type == "subloc":
        ...
        regexes += [j for i in [values for keys, values in expars_allowed.items() if keys in all_types] for j in i]
...
if len(consistency_df["exPar error"]) == 0:
    ...
    logging.warning(f"{len(consistency_df['exPar error'])} locaties met ExPar errors")

if len(consistency_df["intLoc missing"]) == 0:
    logging.info("alle interne locaties uit idmap opgenomen in locationSets")
else:
    logging.warning(f"{len(consistency_df['intLoc missing'])} interne locaties niet opgenomen in locationSets")

# expar missings
logging.info("controle missende ex-parameters")
ex_par_missing = {"internalLocation": [], "exPars": [], "QR": [], "QS": [], "HS": []}
...
if len(consistency_df["exPar missing"]) == 0:
    ...
    logging.warning(f"{len(consistency_df['exPar missing'])} locaties met ExPar missing")
    ...
    if "exLoc_ignore" in consistency_df.keys():
        if int(ex_loc) in consistency_df["exLoc_ignore"]["externalLocation"].values:
            int_loc_error = [
                ...
                if not int_loc
                in consistency_df["exLoc_ignore"][consistency_df["exLoc_ignore"]["externalLocation"] == int(ex_loc)][
                    "internalLocation"
                ].values
            ]
...
if len(consistency_df["exLoc error"]) == 0:
    ...
    logging.warning(f"{len(consistency_df['exLoc error'])} externe locaties onlogisch bij interne locaties")
...
idmap_subloc_df = idmap_df[
    idmap_df["internalLocation"].isin(subloc_gdf["LOC_ID"].values)
]  # alleen locaties die in de sub-locs locationSet zitten
idmap_subloc_df["type"] = idmap_subloc_df["internalLocation"].apply(
    (lambda x: subloc_gdf[subloc_gdf["LOC_ID"] == x]["TYPE"].values[0])
)  # toevoegen van type
...
for loc_group, group_df in idmap_subloc_df.groupby("loc_groep"):
    ...
    ex_locs_dict = {
        k: (ex_locs_dict[k[1:]] if (k[1:] in ex_locs_dict.keys()) and (not k in split_ts) else v)
                ...
                for idx, series in enumerate(sp_series):
                    ...
                    if len(int_par) > 1:
                        ...
                        ts_errors["fout"].append(
                            f'{",".join(int_par)} gekoppeld aan 1 sp-serie (exPar: {ex_par}, exLoc(s)): {",".join(ex_locs)}'
                        )
                    ...
                    if len(conflicting_pars) > 0:
                        ...
                        ts_errors["type"].append(sub_type)
                        ts_errors["fout"].append(
                            f'{",".join(conflicting_pars)} gekoppeld aan sp-serie (exPar: {ex_par}, exLoc(s)): {",".join(ex_locs)}'
                        )
...
if len(consistency_df["timeSeries error"]) == 0:
    ...
    logging.warning(f"{len(consistency_df['timeSeries error'])} tijdseries missend/onlogisch gekoppeld")

# controle validationrulesets
logging.info("controle validationRules")

# TODO: df_idmap moet IDoppervlaktewater + Hymos bevatten
# TODO: kolom fout_type en fout_opmerking toevoegen/splitsen
# TODO: internal parameters uniek maken
# TODO: fout melden als smin/smax lijsten niet unieke waarden bevatten
# TODO: comment regel in ini-file bij lijst-vergelijkingen. Herstructureren json zodat die fout niet gemaakt kan worden?
...
for set_name in config["validation_rules"].keys():
    ...
    location_set_meta = next(loc_set for loc_set in location_sets_dict if loc_set["id"] == location_set["id"])[
        "csvFile"
    ]
    ...
    for attrib_file in attrib_files:
        ...
        attrib_df = pd.read_csv(
            fews_config.MapLayerFiles[attrib_file["csvFile"].replace(".csv", "")], sep=None, engine="python",
        )
    ...
    for (idx, row) in location_set_gdf.iterrows():
        ...
        attribs_obsolete = [
            attrib for attrib in validaton_attributes if (not attrib in attribs_required) and (attrib in row.keys())
        ]
...
if len(consistency_df["validation error"]) == 0:
    ...
    logging.warning(f"{len(consistency_df['validation error'])} validatieregels zijn fout/missend")
...
for idx, row in idmap_df.iterrows():
    ...
    ext_par = [
        mapping["external"]
        for mapping in config["parameter_mapping"]
...
if len(consistency_df["par mismatch"]) == 0:
    ...
    logging.warning(f"{len(consistency_df['par mismatch'])} regex fouten op interne en externe parameters")
...
# TODO: missing_in_set: - kijken in welke set een locatie hoort te zitten en of dat klopt met de sectie?
# TODO: missing_in_set: - kijken of de locatie mist in de locatieset. Even een locatie uit de csv halen om te testen.
    ...
    for idmap in ["IdOPVLWATER", "IdOPVLWATER_HYMOS"]:
        for section in idmap_sections[idmap][section_name]:
            int_locs += [
                ...
                for item in xml_to_dict(fews_config.IdMapFiles[idmap], **section)["idMap"]["map"]
            ]
...
if len(consistency_df["locSet error"]) == 0:
    ...
    logging.warning(f"{len(consistency_df['locSet error'])} fouten in locationSets")
...
for cell in worksheet[f"{worksheet.max_row}"]:
...
for locationSet, gdf in {
    "OPVLWATER_HOOFDLOC": hoofdloc_gdf,
    ...
    if locationSet == "OPVLWATER_WATERSTANDEN_AUTO":
        ...
        df[["HBOVPS", "HBENPS"]] = df.apply(update_peilschaal, axis=1, result_type="expand")
